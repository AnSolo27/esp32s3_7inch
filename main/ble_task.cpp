...
void BLE_Task(void* arg) {
    //lv_obj_add_event_cb(
    //guider_ui.screen_main_btn_1, event_handler, LV_EVENT_ALL, NULL);

    while(1) {
        int i = 1;
        vector<std::pair<string, string> > elems(
            ble_devices.begin(), ble_devices.end());
        sort(elems.begin(), elems.end(), comp);
        for(auto el : elems) {
            //lv_table_set_cell_value(
            //    guider_ui.screen_main_table_1, i, 0, el.first.c_str());
            //lv_table_set_cell_value(
            //    guider_ui.screen_main_table_1, i, 1, el.second.c_str());
            i++;
            if(i > 10) {
                break;
            }
