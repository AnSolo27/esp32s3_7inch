 ...
 */
static void ble_spp_client_on_disc_complete(
...
static int ble_spp_client_should_connect(const struct ble_gap_disc_desc* disc) {
    ...
    for(i = 0; i <= CONFIG_BT_NIMBLE_MAX_CONNECTIONS; i++) {
        if(memcmp(&connected_addr[i].val, disc->addr.val, PEER_ADDR_VAL_SIZE) ==
           0) {
 ...
 */
static void ble_spp_client_connect_if_interesting(
    ...
    rc = ble_gap_connect(
        own_addr_type, &disc->addr, 30000, NULL, ble_spp_client_gap_event, NULL);
    if(rc != 0) {
        MODLOG_DFLT(
            ERROR,
...
static int ble_spp_client_gap_event(struct ble_gap_event* event, void* arg) {
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        if(event->connect.status == 0) {
            ...
            assert(rc == 0);
            memcpy(
                &connected_addr[event->connect.conn_handle].val,
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        ...
        /* Forget about peer. */
        memset(
            &connected_addr[event->disconnect.conn.conn_handle].val,
            0,
            PEER_ADDR_VAL_SIZE);
        ...
        if(xQueueReceive(
            ...
            case UART_DATA:
                if(event.size) {
                    ...
                    if(temp == NULL) {
                        ESP_LOGE(
                            tag, "malloc failed,%s L#%d", __func__, __LINE__);
                        break;
                    }
                    memset(temp, 0x0, event.size);
                    uart_read_bytes(
                        UART_NUM_0, temp, event.size, portMAX_DELAY);
                    for(i = 0; i <= CONFIG_BT_NIMBLE_MAX_CONNECTIONS; i++) {
                        if(attribute_handle[i] != 0) {
                            ...
                            if(rc == 0) {
                                ...
                                ESP_LOGI(
                                    tag,
                                    "Error in writing characteristic rc=%d",
                                    rc);
                            }
                            vTaskDelay(10);
                        }
...
static void ble_spp_uart_init(void) {
    ...
    uart_set_pin(
        ...
        UART_PIN_NO_CHANGE);
    xTaskCreate(
        ble_client_uart_task, "uTask", 4096, (void*)UART_NUM_0, 8, NULL);
}
